...
void initArgv(Argv *a, size_t initialSize) {
  a->args = malloc(initialSize * sizeof(char*));
  a->used = 0;
  a->size = initialSize;
}

//insert elements to the dynamic array
void insertArgv(Argv *a, char* element) {
  if (a->used == a->size) {
    a->size *= 2;
    a->args = realloc(a->args, a->size * sizeof(char*));
  }
  a->args[a->used++] = element;
}

//free dynamic array
void freeArgv(Argv *a) {
  free(a->args);
  a->args = NULL;
  a->used = a->size = 0;
}
